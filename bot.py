...
client = commands.Bot(description="RoyBot Is Awesome", command_prefix=commands.when_mentioned_or("roy!" ,"ROY!"))
...
async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='roy!help', type=3))
...
async def dm(ctx, user: discord.Member, *, msg: str):
    if ctx.message.author.bot:
        ...
        if msg is None:
            await client.say('Invalid command. Use this command like: ``ROY\\roy!dm @user message``') 	
        else:
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
...
client.run('NjIzMDUzMzAwNjE0Mjk5NjQ4.XX81rg.K1mzswRBxqe_o1VgGWQbZ5ZxoKE')
